...
public class PlatformMovement3D : MonoBehaviour {
    ...
    void OnTriggerEnter (Collider other) {
        ...
        if (other.CompareTag("PickUp")) {
            PickUpRotator pickUp = other.GetComponent<PickUpRotator>();
            if (playerScript.Pick != null || playerScript.currentPower != pickUp)
            {
                if (playerScript.Pick != null && playerScript.currentPower != pickUp)
                {
                    Destroy(playerScript.Pick.gameObject);
                }
                playerScript.Pick = pickUp;
                
                QuestManager.instance.Check("obtain", pickUp.PickUpName);
            }
        }
    }
